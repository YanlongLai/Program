...
bool test0(void)
{
    ...
    *d_KernelSpectrum;
    ...
    const int kernelY = 3;
    const int kernelX = 4;
    const int   dataH = 4096;
    const int   dataW = 4096;
    ...
    GPU_data = (float *)malloc(fftH    * fftW * sizeof(float));
    ...
    {
        h_Kernel[i] = getRand();
        //printf("%2.f\t",h_Kernel[i]);
        //if(i % kernelW == kernelW-1)
        //printf("\n");
    }
    ...
    checkCudaErrors(cudaMemcpy(GPU_data, d_DataSpectrum, fftH * fftW * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for (int y = 0; y < fftH; y++){
        ...
        {
            double fft_data = (double)GPU_data[ y * fftW + x];
            //printf("%2.f\t",fft_data);
    ...
    for (int y = 0; y < dataH; y++){
        for (int x = 0; x < dataW; x++)
        {
            ...
            double fft_data = (double)GPU_data[ y * dataW + x];
            
            //printf("%2.f\t",fft_data);
            //printf("%2.f\t", 100*log(1+std::abs((int)fft_data)));
